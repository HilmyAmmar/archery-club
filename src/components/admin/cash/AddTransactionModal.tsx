...
import { X, Loader2, Save, UploadCloud, Calendar, DollarSign, FileText, Wallet, Tag, ArrowRightLeft } from 'lucide-react';
...
interface TransactionModalProps {
  ...
  initialData?: any; // Kalau ada isinya, berarti mode Edit
}

// ============================================================================
// KONFIGURASI KATEGORI (DROPDOWN LIST)
// ============================================================================
...
export default function AddTransactionModal({ isOpen, onClose, onSuccess, initialData }: TransactionModalProps) {
  ...
  // Set Data kalau mode Edit atau Buka Modal Baru
  useEffect(() => {
  ...
  const handleUploadImage = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      const filePath = `manual/${fileName}`; // Masuk ke folder manual/
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // Kalau user ganti tipe (Masuk -> Keluar), reset kategorinya biar nggak nyangkut
    if (name === 'tipe') {
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold">{initialData ? 'Edit Transaksi Kas' : 'Catat Transaksi Baru'}</h2>
                <p className="text-xs text-slate-500 font-medium">Input arus kas manual (Sewa alat, operasional, dll)</p>
              </div>
            </div>
          ...
          <div className="p-6 overflow-y-auto flex-1 custom-scrollbar text-slate-800">
            <form id="cash-form" onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input required type="date" name="tanggal" value={formData.tanggal} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:border-blue-500 outline-none" />
              </div>

              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold flex items-center gap-2"><ArrowRightLeft className="w-4 h-4 text-slate-400" /> Tipe Transaksi</label>
                <select name="tipe" value={formData.tipe} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:border-blue-500 outline-none cursor-pointer">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold flex items-center gap-2"><Tag className="w-4 h-4 text-slate-400" /> Kategori</label>
                <div className="relative">
                  ...
                  >
                    ...
                    {/* Tampilkan kategori custom/lama jika sedang mode Edit */}
                    {isCustomCategory && (
                      <option value={formData.kategori}>{formData.kategori} (Data Lama)</option>
                    )}

                    {/* Tampilkan list standar sesuai tipe (Pemasukan/Pengeluaran) */}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold flex items-center gap-2"><DollarSign className="w-4 h-4 text-slate-400" /> Nominal</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-bold">Rp</span>
                  <input required type="number" name="nominal" placeholder="0" value={formData.nominal} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-4 py-3 font-bold focus:border-blue-500 outline-none" />
                </div>
              </div>

              {/* UPLOAD FOTO SECTION */}
              <div className="md:col-span-2">
                <label className="text-sm font-semibold flex items-center gap-2 mb-2"><UploadCloud className="w-4 h-4 text-slate-400" /> Bukti Transaksi (Opsional)</label>
                <div className="flex flex-col sm:flex-row items-center gap-4 p-4 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    ...
                    <div className="flex gap-2 justify-center sm:justify-start">
                      <label htmlFor="bukti_transaksi_manual" className="cursor-pointer px-4 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold hover:bg-blue-50 transition-all">
              ...
              <div className="md:col-span-2 text-slate-800 font-bold">
                <label className="text-sm font-semibold flex items-center gap-2 mb-1.5"><FileText className="w-4 h-4 text-slate-400 " /> Keterangan Tambahan</label>
                <textarea name="keterangan" placeholder="Detail transaksi..." value={formData.keterangan} onChange={handleChange} rows={2} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-blue-500 outline-none resize-none" />
              </div>
            </form>
          </div>
          ...
          <div className="p-6 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3 shrink-0">
            ...
            <button type="submit" form="cash-form" disabled={loading || uploading} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 shadow-lg shadow-blue-200 active:scale-95 disabled:opacity-50">
      ...
      {showLightbox && imagePreview && (
        <div className="fixed inset-0 z-[1001] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300 cursor-zoom-out" onClick={() => setShowLightbox(false)}>
          <div className="relative max-w-4xl max-h-[90vh] animate-in zoom-in-95 duration-300">
            ...
            <button onClick={() => setShowLightbox(false)} className="absolute -top-4 -right-4 p-2 bg-white rounded-full shadow-lg">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
