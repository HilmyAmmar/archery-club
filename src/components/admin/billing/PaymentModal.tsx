...
import { X, Loader2, Save, Receipt, UploadCloud, Calendar, DollarSign, FileText, AlertCircle } from 'lucide-react';
...
export default function PaymentModal({ isOpen, onClose, billingData, onSave, isUpdating }: PaymentModalProps) {
  ...
  const handleUploadImage = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      setValidationError(`Gagal mengunggah gambar: ${error.message}`);
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    // --- DEFENSE: LOGIC VALIDATION ---
    if (name === 'nominal_bayar') {
      const val = Number(value);
      if (val < 0) return; // Langsung blokir kalau minus

      if (val > billingData.nominal_tagihan) {
        setValidationError(`Ups! Nominal melebihi tagihan (Maks: Rp ${billingData.nominal_tagihan.toLocaleString('id-ID')})`);
      } else {
        setValidationError(null);
      }
    }
  ...
  // --- DEFENSE: KEYBOARD BLOCKER ---
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // Blokir karakter yang tidak diinginkan dalam nominal uang
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const nominalInput = Number(formData.nominal_bayar);
    const totalTagihan = billingData.nominal_tagihan;

    if (nominalInput > totalTagihan) {
      setValidationError(`Pembayaran tidak dapat diproses karena nominal melebihi tagihan.`);
      return; 
    }

    const finalData = {
      ...formData,
      nominal_bayar: nominalInput,
      tanggal_bayar: nominalInput === 0 ? null : formData.tanggal_bayar,
      bukti_transaksi: nominalInput === 0 ? null : formData.bukti_transaksi
    };

    onSave({ id: billingData.id, tagihan: totalTagihan, ...finalData });
  };
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-800">Pembaruan Pembayaran</h2>
                <p className="text-xs text-slate-500 font-medium">{billingData.members?.nama_lengkap}</p>
              </div>
            </div>
            <button onClick={onClose} disabled={isUpdating || uploading} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
            ...
            {validationError && (
              <div className="mb-6 p-4 bg-rose-50 border border-rose-100 rounded-xl flex items-center gap-3 text-rose-700 animate-in slide-in-from-top-2">
            ...
            <form id="payment-form" onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-bold text-slate-700 flex items-center gap-2">
                  <DollarSign className="w-4 h-4 text-slate-400" /> Nominal Dibayar
                </label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder="0"
                    className={`w-full bg-slate-50 border rounded-xl pl-11 pr-4 py-3 font-bold focus:ring-1 outline-none transition-all ${
                      validationError ? 'border-rose-300 focus:border-rose-500 ring-rose-100' : 'border-slate-200 focus:border-blue-500 ring-blue-100'
                    }`} 
                  />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input 
                  ...
                  onChange={handleChange} 
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:border-blue-500 outline-none font-medium text-slate-700" 
                />
              </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 p-4 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    ...
                    <div className="flex gap-2 justify-center sm:justify-start">
                      <label htmlFor="bukti_transaksi" className="cursor-pointer px-4 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold hover:bg-blue-50 transition-all shadow-sm text-slate-700">
                        {imagePreview ? 'Ganti Gambar' : 'Pilih Gambar'}
                      </label>
                      {imagePreview && (
                        <button type="button" onClick={handleRemoveImage} className="px-4 py-2 bg-white border border-rose-200 text-rose-600 rounded-lg text-xs font-bold hover:bg-rose-50 transition-colors">
                          Hapus
                        </button>
                      )}
                    </div>
              ...
              <div className="md:col-span-2">
                <label className="text-sm font-bold text-slate-700 flex items-center gap-2 mb-1.5">
                  <FileText className="w-4 h-4 text-slate-400" /> Catatan
                </label>
                <textarea 
                  name="keterangan" 
                  ...
                  onChange={handleChange} 
                  rows={2} 
                  placeholder="Contoh: Bayar via transfer Bank Mandiri..."
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-blue-500 outline-none resize-none font-medium text-slate-700" 
                />
              </div>
            </form>
          </div>
          ...
          <div className="p-6 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3 shrink-0">
            <button onClick={onClose} disabled={isUpdating} className="px-5 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-200 rounded-xl transition-all">
              Batal
            </button>
            <button 
              type="submit" 
              form="payment-form" 
              disabled={isUpdating || uploading || !!validationError} 
              className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 shadow-lg shadow-blue-200 active:scale-95 disabled:opacity-50 transition-all"
            >
      ...
      {showLightbox && imagePreview && (
        <div className="fixed inset-0 z-[1001] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300 cursor-zoom-out" onClick={() => setShowLightbox(false)}>
          <div className="relative max-w-4xl max-h-[90vh] animate-in zoom-in-95 duration-300">
            <img src={imagePreview} className="rounded-2xl shadow-2xl border-4 border-white/20 max-h-[85vh] object-contain" onClick={(e) => e.stopPropagation()} alt="Preview Full" />
